...
page = st.selectbox("Choose your module", ["EDA", "Data preprocessing", "Auto ML"]) 
				...
				if st.sidebar.button('Process Export to Oracle'):
					with open('temp.csv', 'r+') as f:
						st.sidebar.download_button(
								...
								data=f,
								mime='text/csv',
								file_name="OracleConvertedFile.csv",
								)
	...
	if package:
		...
		try:
			...
			if plt=='Pandas Profiling':
				...
				df.to_csv('data.csv')
				sv = sv.analyze(df)
				sv.show_html("SWEETVIZ_REPORT.html")
				...
				if st.sidebar.button('Process Dtale'):
					...
					# LOOK amirtha LOOK, remember we set the data_id of our previous dataframe was "1"
		...
		try:
			...
			if downl=='Download as CSV':
				...
				if st.sidebar.button('Process Export to Oracle'):
					with open('temp.csv', 'r+') as f:
						st.sidebar.download_button(
								label="Download Oracle-XLSX",
								data=f,
								mime='text/xlsx',
								file_name="OracleConvertFile.xlsx",
								)
				...
				if st.sidebar.button('Process Push As Email Attachment'):
					with open('temp.csv', 'r+') as f:
						st.sidebar.download_button(
								...
								data=f,
								mime='text/xlsx',
								file_name="EmailAttachmentFile.xlsx",
								)
	...
	if ml:
		...
		try:
			...
			if upload=="Naive Bayes":
				...
				if target:
					if st.sidebar.button('Process Naive Bayes'):
						y=df[target]
						x=df
						st.write("Target")
						st.dataframe(y)
			...
			if upload=="Logistic Classification":
				df=pd.read_excel(uploaded_file)
				df=df.drop(df.columns[0], axis=1)
				columns=df.columns
				target=st.sidebar.selectbox("Choose The columns as Target",columns)
			...
			if upload=="Support Vector Machine":
				df=pd.read_excel(uploaded_file)
				df=df.drop(df.columns[0], axis=1)
				columns=df.columns
				target=st.sidebar.selectbox("Choose The columns as Target",columns)
			...
			if upload=="EAML-AutoRun":
				...
				if st.button("Save Pickle File"):
					st.info("Your Model has been saved as 'best_model.pkl', to prevent file overwrite, please rename and save")
					st.download_button(label="Export Pickle File", data=y, file_name=f"REPORT_{todaydate}at{current_time}.pkl")
					st.rerun()
